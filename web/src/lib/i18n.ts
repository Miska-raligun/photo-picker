...
const enMessages = {
  en: {
    common: {
      ...
      firstRunHint: "Pick a folder of photos below — that's all you need to start your first scan.",
    },
    scanForm: {
      title: "New task",
      source: "Source",
    ...
    settings: {
      ...
      notifyDesc: "Browser permission required. Quietly skipped when the tab is in the foreground.",
      notifyToggle: "Notify me when a scan finishes",
      notifyUnsupported: "This browser doesn't support web notifications.",
      notifyPermissionDenied: "Browser denied notification permission.",
      tokenHeading: "Server access token",
      tokenDesc:
        "Required only when the server was started with PHOTO_PICK_TOKEN (recommended whenever it listens on anything other than localhost). Stored in this browser.",
      tokenPlaceholder: "leave empty if the server has no token",
    },
    runCard: {
      ...
      cancelTitle: "Stop this scan (already-extracted features stay cached)",
      ...
      starting: "starting…",
      notifyCompleteBody: (n: number) =>
        `Scan complete — ${n} ${n === 1 ? "photo" : "photos"} processed.`,
    ...
    detail: {
      ...
      prevGroupBtn: "Previous group (PgUp)",
      nextGroupBtn: "Next group (PgDn)",
      groupUnavailable: "This group couldn't be loaded.",
      sortBy: "Sort",
      sortAlgo: "Algo rank",
      sortAi: "AI rank",
      sortScore: "Final score",
      ...
      filterLowIso: "Low ISO ≤ 800",
      filterEmpty: "No photos match the current filter.",
      bulkSelected: (n: number) => `${n} selected`,
      bulkKeep: "Keep",
      bulkReject: "Reject",
      bulkReset: "Reset",
      bulkClear: "Clear selection (Esc)",
      filterFlagged: "⚑ Flagged",
      flag: "Flag this photo",
      unflag: "Remove flag",
      flagged: "Flagged",
      notePlaceholder: "Add a note (e.g. \"fix crop\", \"send to client\")…",
      cardSize: "Card size",
      findSimilar: "Find similar",
      similarLoading: "Searching…",
      similarNone: "No similar photos found in this run.",
    },
    duplicates: {
      title: "Exact duplicates",
      scan: "Scan",
      rescan: "Rescan",
      summary: (groups: number, redundant: number) =>
        `${groups} set(s) · ${redundant} redundant file(s)`,
      none: "No byte-identical duplicates in this run.",
    },
    shortcuts: {
      title: "Keyboard shortcuts",
      groupGrid: "Photo grid",
      groupLightbox: "Lightbox",
      groupGlobal: "Global",
      gridClick: "Toggle keep / delete",
      gridShift: "Select a range",
      gridCtrl: "Add or remove from selection",
      gridEsc: "Clear selection",
      gridArrows: "Previous / next composition group",
      lbArrows: "Previous / next photo",
      lbHomeEnd: "First / last photo in the group",
      lbPage: "Previous / next composition group",
      lbSpace: "Toggle keep / delete",
      lbInfo: "Toggle the details panel",
      lbZoom: "Zoom in / out / reset",
      lbWheelKey: "Wheel / double-click",
      lbWheel: "Zoom at the cursor",
      lbEsc: "Close",
      globalHelp: "Show this help",
    },
    compare: {
      title: "Compare with another scan",
      pickRun: "Pick a run to compare against…",
      keptCounts: (here: number, there: number) => `kept ${here} vs ${there}`,
      identical: "Both runs kept exactly the same photos.",
      addedKept: (n: number) => `+${n} kept here but not there`,
      removedKept: (n: number) => `−${n} kept there but not here`,
      notInOther: "not present in the other run",
      fileDelta: (here: number, there: number) =>
        `File set differs too: ${here} photo(s) only in this run, ${there} only in the other.`,
    },
    export: {
      button: "Export…",
    ...
    applyBar: {
      ...
      toastReveal: "Show in folder",
      toastRevealFailed: "Could not open file manager",
      toastManifestSaved: (p: string) => `Audit log saved to ${p}`,
    },
    browse: {
...
export const messages: Record<Lang, Messages> = {
  ...
  zh: {
    common: {
      ...
      firstRunHint: "在下面选择一个照片文件夹，就能开始第一次扫描。",
    },
    scanForm: {
      title: "新建任务",
      source: "源目录",
    ...
    settings: {
      ...
      notifyHeading: "通知",
      notifyDesc: "需要浏览器授权；当前页面已在前台时会自动跳过。",
      notifyToggle: "扫描完成时桌面通知",
      notifyUnsupported: "当前浏览器不支持桌面通知。",
      notifyPermissionDenied: "浏览器拒绝了通知权限。",
      tokenHeading: "服务访问令牌",
      tokenDesc:
        "仅当服务端设置了 PHOTO_PICK_TOKEN 时需要填（监听地址不是 localhost 时强烈建议开启）。保存在本浏览器。",
      tokenPlaceholder: "服务端未设令牌则留空",
    },
    runCard: {
      scanInProgress: "扫描中",
      scanComplete: "扫描完成",
      ...
      cancel: "取消",
      cancelling: "取消中…",
      cancelTitle: "停止本次扫描(已提取的特征仍会保留在缓存)",
      cancelFailed: "取消失败",
      inPlace: "原地模式",
      statPhotos: "总数",
      statCache: "缓存命中",
      ...
      starting: "启动中…",
      notifyCompleteBody: (n: number) => `扫描完成 · ${n} 张照片`,
    ...
    detail: {
      ...
      prevGroup: "上一组（←）",
      nextGroup: "下一组（→）",
      prevGroupBtn: "上一构图组（PgUp）",
      nextGroupBtn: "下一构图组（PgDn）",
      ...
      filterKept: "保留",
      filterRejected: "拒绝",
      filterOverridden: "已覆盖",
      filterLowIso: "低 ISO ≤ 800",
      filterEmpty: "当前筛选下没有照片。",
      ...
      unflag: "取消旗标",
      flagged: "已旗标",
      notePlaceholder: "写点备注（如\"要裁剪\"\"发客户\"）…",
      cardSize: "卡片大小",
      findSimilar: "查找相似",
      similarLoading: "搜索中…",
      similarNone: "本次扫描中没有找到相似照片。",
    },
    duplicates: {
      title: "完全重复文件",
      scan: "检测",
      rescan: "重新检测",
      summary: (groups: number, redundant: number) =>
        `${groups} 组 · ${redundant} 个冗余文件`,
      none: "本次扫描没有字节完全相同的重复文件。",
    },
    shortcuts: {
      title: "键盘快捷键",
      groupGrid: "照片网格",
      ...
      gridCtrl: "加入或移出选区",
      gridEsc: "清空选区",
      gridArrows: "上一组 / 下一组构图",
      lbArrows: "上一张 / 下一张",
      lbHomeEnd: "本组第一张 / 最后一张",
      lbPage: "上一组 / 下一组构图",
      lbSpace: "切换保留 / 删除",
      lbInfo: "切换详情面板",
      lbZoom: "放大 / 缩小 / 重置",
    ...
    compare: {
      title: "与另一次扫描对比",
      pickRun: "选择要对比的任务…",
      keptCounts: (here: number, there: number) => `保留 ${here} vs ${there}`,
      identical: "两次扫描保留的照片完全一致。",
      addedKept: (n: number) => `+${n} 本次保留、对方未保留`,
      removedKept: (n: number) => `−${n} 对方保留、本次未保留`,
      notInOther: "对方扫描中不存在此文件",
      fileDelta: (here: number, there: number) =>
        `文件集合也有差异：${here} 张仅在本次，${there} 张仅在对方。`,
    },
    export: {
      button: "导出…",
      title: "导出保留的照片",
    ...
    applyBar: {
      ...
      toastReveal: "在文件管理器中显示",
      toastRevealFailed: "打开文件管理器失败",
      toastManifestSaved: (p: string) => `删除清单已保存到 ${p}`,
    },
    browse: {
